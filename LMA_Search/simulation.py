...
class PPOEnv(gym.Env):
    ...
    def __init__(self, grid_size=500, num_cca=4): # Default to 4 CCAs
        super().__init__()
        self.grid_size = grid_size
        if num_cca != 4: raise ValueError("This environment requires num_cca=4")
        self.num_cca = num_cca
        self.observable_radius = 100.0
        self.history_len = 200 # Observation history length

        # Simulation state
        self.current_step = 0
        self.max_steps = 1000 # Increased max steps for harder task?
        self.foxtrot_obj = None
        self.cca_objs = []

        # Reward/State Tracking
        self.previous_cca_positions = None # Store previous positions for progress/movement calc
        self.was_observable_prev_step = False
        self.cell_visit_counts = {}
        self.grid_resolution = 25 # Cell size for revisit penalty (tune)

        # Action Space (4 agents, 3D action)
        self.action_space = spaces.Box(
            low=-step_size, high=step_size, shape=(self.num_cca, 3), dtype=np.float32
        )

        # Observation Space
        self.observation_dim_per_step = (3 + 3) * self.num_cca + 3 # (pos+act)*N + fox_pos = (6*4)+3=27
        self.observation_shape_total = self.history_len * self.observation_dim_per_step # 200 * 27 = 5400
        low_bound = -max(float(self.grid_size), float(step_size), 1.0)
        ...
        self.cca_collision_radius = 5.0 # Radius for direct collision penalty

        print(f"\nPPOEnv Initialized (4 Agents, Partial Obs, L={self.history_len}):")
        print(f"  Grid: {self.grid_size}x{self.grid_size}x{self.grid_size}")
        print(f"  Obs Radius: {self.observable_radius}, Capture Radius: {self.capture_radius}")
    ...
    def reset(self, seed=None, options=None):
        ...
        # Reset CCAs (Random Positions, avoid overlap)
        self.cca_objs = []
        min_start_sep = 10.0
        for i in range(self.num_cca):
            ...
            while attempts < 100: # Prevent infinite loop
                 ...
                 is_too_close = any(np.linalg.norm(initial_cca_pos - cca.position) < min_start_sep for cca in self.cca_objs)
        ...
        self.previous_cca_positions = [cca.position.copy() for cca in self.cca_objs] # Store initial positions
    ...
    def step(self, actions):
        ...
        for i in range(self.num_cca):
            ...
            current_cca_positions.append(self.cca_objs[i].position.copy()) # Store current pos
        ...
        reward = self._calculate_reward(
            ...
            current_cca_positions=current_cca_positions
        )
    ...
    def _get_observation(self):
        ...
        for t in range(self.history_len):
            for i in range(self.num_cca):
                ...
                state_components.append(self.action_history[i][t]) # Action taken @ t-1
            state_components.append(self.foxtrot_history[t])   # Foxtrot obs @ t
        obs_flat = np.concatenate(state_components).astype(np.float32)
        if obs_flat.shape != (self.observation_shape_total,):
             raise ValueError(f"Obs shape mismatch! Exp {(self.observation_shape_total,)}, Got {obs_flat.shape}")
        return obs_flat

    def _calculate_reward(self, actions=None, is_observable_now=False, current_cca_positions=None):
        """
        ...
        """
        # --- Parameters (Tune!) ---
        REWARD_RANGE = 500.0         # Target magnitude for mode-specific rewards
        # Intercept (Visible) Weights
        w_progress = 1.0             # Weight for normalized progress towards target
        # Search (Hidden) Weights
        ...
        find_bonus = 100.0
        time_penalty = -0.1
        cca_collision_penalty = -20.0
        energy_penalty_coeff = 0.005
        # Normalization helpers
        max_dist_change_norm = 1.0 * step_size
        proximity_threshold = self.cca_collision_radius * 10
        max_revisit_penalty_norm = 5.0
        separation_norm_factor = self.grid_size # For normalizing separation score
        ...
        # --- Calculate Distances ---
        current_individual_distances = [np.linalg.norm(pos - current_foxtrot_position) for pos in current_cca_positions]
        current_avg_distance = np.mean(current_individual_distances) # <<< CALCULATE AVG DISTANCE HERE
        min_dist_to_foxtrot = min(current_individual_distances)

        avg_cca_separation = 0.0
        ...
        # --- Initialize Reward ---
        reward = 0.0
        mode_reward_scaled = 0.0
        prog_r_scaled = 0.0; sep_p_scaled = 0.0; rev_p_scaled = 0.0 # For debug

        # --- Mode-Dependent Rewards ---
        if is_observable_now:
            # --- Mode 1: Intercept ---
            mode = "Intercept"
            total_progress_norm = 0.0
            num_valid_prev = 0
            # Ensure previous positions are available and match current number of agents
            if self.previous_cca_positions is not None and len(self.previous_cca_positions) == self.num_cca:
                for i in range(self.num_cca):
                    # Calculate previous distance using stored previous position
                    prev_dist = np.linalg.norm(self.previous_cca_positions[i] - current_foxtrot_position)
                    curr_dist = current_individual_distances[i] # Use pre-calculated current distance

                    individual_distance_reduction = prev_dist - curr_dist
                    normalized_ind_progress = np.clip(individual_distance_reduction / (max_dist_change_norm + 1e-6), -1.0, 1.0)
                    total_progress_norm += (normalized_ind_progress + 1.0) / 2.0 # Map to [0, 1]
                    num_valid_prev += 1

            if num_valid_prev > 0:
                avg_progress_norm = total_progress_norm / num_valid_prev
                mode_reward_scaled = w_progress * avg_progress_norm * REWARD_RANGE
                prog_r_scaled = mode_reward_scaled # Store for debug
            ...
            # --- Mode 2: Search ---
            mode = "Search"
            # 1. Proximity Penalty Score [0, 1] (higher means closer/worse)
            ...
            # 2. Re-exploration Penalty Score [0, 1] (higher means more revisited)
            ...
            for pos in current_cca_positions:
                ...
                if cell not in current_cells:
                    ...
                    revisit_penalty_raw += math.sqrt(visit_count)
                    self.cell_visit_counts[cell] = visit_count + 1
                    current_cells.add(cell)
            revisit_score_norm = np.clip(revisit_penalty_raw / (max_revisit_penalty_norm * self.num_cca + 1e-6), 0.0, 1.0)

            # Calculate total *negative* reward, scaled to approx [-REWARD_RANGE, 0]
            proximity_penalty_scaled = - (w_proximity * proximity_score_normalized * REWARD_RANGE)
            revisit_penalty_scaled = - (w_revisit * revisit_score_norm * REWARD_RANGE)
            mode_reward_scaled = proximity_penalty_scaled + revisit_penalty_scaled
            sep_p_scaled = proximity_penalty_scaled # Store for debug
            rev_p_scaled = revisit_penalty_scaled   # Store for debug
            reward += mode_reward_scaled

        # --- Shared Components ---
        if min_dist_to_foxtrot < self.capture_radius: reward += capture_bonus
        reward += time_penalty
        if actions is not None: reward -= energy_penalty_coeff * np.sum(actions**2)
        if self.num_cca > 1 and min_cca_dist < self.cca_collision_radius: reward += cca_collision_penalty

        # --- Debug Printing ---
        if REWARD_DEBUG:
             # current_avg_distance IS defined now
             ...
             energy_p = -energy_penalty_coeff * np.sum(actions**2) if actions is not None else 0

             # prog_r_scaled, sep_p_scaled, rev_p_scaled are already calculated above for debug

             print(f"Step {self.current_step}: Total={reward:.2f} | M={mode} | Dists=[{dist_str}] | CCAs=[{cca_dist_str}] | "
                   f"ModeRew={mode_reward_scaled:.1f} (ProgR={prog_r_scaled:.1f} | ProxP={sep_p_scaled:.1f} | RevisP={rev_p_scaled:.1f}) | "
...
if __name__ == "__main__":
    ...
    lma_kwargs = dict(
        ...
        ff_latent_hidden=128*4,   # Latent MLP hidden (e.g., 2*d_new)
        num_lma_layers=4,       # More layers for longer sequence and harder task
        seq_len=100,            # Initial Sequience length (L=100)
        dropout=0.1,
        bias=True
    )
    ...
    policy_kwargs = dict(
        ...
        net_arch=dict(pi=[256, 256, 128], vf=[256, 256, 128, 64]) # Example larger arch
    )
